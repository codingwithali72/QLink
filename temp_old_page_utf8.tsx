...
export default function ReceptionPage({ params }: { params: { clinicSlug: string } }) {
    ...
    // ΓöÇΓöÇ Per-action loading flags ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // B1: Stalled queue detection ΓÇö track how long since last NEXT
    ...
    // ΓöÇΓöÇ Generic action wrapper with optimistic UI support ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const performAction = async (
        ...
        try {
            ...
            // Do NOT call refresh() on success ΓÇö realtime subscription fires automatically
        } catch (e) {
            if (rollback) rollback();
            console.error(e);
    ...
    // ΓöÇΓöÇ Action handlers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    const handleRecall = (id: string) => {
        ...
        performAction(
            ...
            setNextLoading, // reuse nextLoading ΓÇö recall is a queue-advance variant
    ...
    // B1: Stall detection ΓÇö track when serving token changes
    useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 p-3 pb-20 md:p-6 lg:p-8 font-sans relative">
            ...
            {/* B1: Stall warning ΓÇö queue has not advanced in 5+ minutes */}
            {stallMinutes >= 5 && servingToken && session?.status === 'OPEN' && (
                <div className={`fixed top-0 left-0 w-full text-white text-center text-xs py-2 font-bold z-50 animate-pulse flex items-center justify-center gap-2 ${stallMinutes >= 10 ? 'bg-red-600' : 'bg-amber-500'
                    ...
                    Queue stalled ΓÇö {stallMinutes} min since last NEXT was pressed. Press NEXT or SKIP to continue.
                </div>
            )}
